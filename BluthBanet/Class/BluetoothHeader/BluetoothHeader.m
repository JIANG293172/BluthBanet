...
//    NSData *data = [self hexToBytes:@"88096F"];
    NSData *data = [self hexToBytes:@"00852"];
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI{
    NSLog(@"peripheral.identifier:%@ peripheral.name: %@ ", [peripheral.identifier UUIDString], peripheral.name);
    //筛选设备名，换成自己的设备名称
    if(([peripheral.name rangeOfString:@"iPhone"].location != NSNotFound) ||([peripheral.name rangeOfString:@"Blood Pressure"].location != NSNotFound) ){
    ...
    if (!error) {
        for (CBService *service in peripheral.services) {
            ...
            if ([service.UUID.UUIDString isEqualToString:ST_SERVICE_UUID]) {
                [self.peripheral discoverCharacteristics:(NSArray *)service.characteristics
                                              forService:service];
                return;
            }
        }
...
//        NSData *cmdData = [data subdataWithRange:NSMakeRange(0, 2)];
        if ([data isEqualToData:[self hexToBytes:CMD_SEND_BATTERYLEVEL_REMIND]]) {
            //设备电量
        ...
        }else if ([data isEqualToData:[self hexToBytes:CMD_SW_REV_INFO]]){
            //设备版本
            unsigned char *bs = (unsigned char *)[[data subdataWithRange:NSMakeRange(2, 1)] bytes];
            unsigned int w1 = *bs;
